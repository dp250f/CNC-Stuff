/**
  ...
  $Date: 2024-11-06 10:45:13 $

  FORKID {154F7C00-6549-4c77-ADE0-79375FE5F2AA}
*/

description = "Grbl with Dust Boot Mod";
vendor = "grbl";
...
properties = {
  ...
  unloadToolNumber: {
    title      : "Unload Tool Number",
    description: "The tool number used to unload current tool without loading a new one.",
    group      : "preferences",
    type       : "integer",
    value      : 0,
    scope      : "post"
  ...
  dustBootParkingClearance: {
    title      : "Dust Boot Parking Clearance",
    description: "-Y distance from center to allow Z movement",
    group      : "Dust Boot",
    type       : "number",
    value      : -25,
    scope      : "post"
  },
  dustBootParkingMagnetOffset: {
    title      : "Dust Boot Parking Magnet Offset",
    description: "+Y distance from center to weaken magnets, making the boot easier to remove",
    group      : "Dust Boot",
    type       : "number",
    value      : 5,
    scope      : "post"
...
function dustBoot(LoadUnload) {
  BootParkingClearance = getProperty("dustBootParkingClearance");
  BootParkingMagnetOffset = getProperty("dustBootParkingMagnetOffset");
  switch (LoadUnload) {
    case "UNLOAD":
      writeComment(localize("Unload dust shoe"));
      writeBlock("; G28 Z0 must be set to most retracted position!!!");
      writeBlock("; Set G30 to dock center location");
      writeBlock(gFormat.format(91), "; Set incremental distance mode");
      writeBlock(gFormat.format(28), "Z0", "; Lift Z to safe position");
      writeBlock(gFormat.format(30), "X0 Y0", "; Move XY to center of Dock");
      writeBlock(gFormat.format(0), yOutput.format(BootParkingClearance), "; Move Y to Dock center " + BootParkingClearance + " for Z clearance"); yOutput.reset();
      writeBlock(gFormat.format(30), "Z0", "; Move Z to dock level");
      writeBlock(gFormat.format(30), "Y0", "; Move Y to center of Dock");
      writeBlock(gFormat.format(0), yOutput.format(BootParkingMagnetOffset), "; Move Y+" + BootParkingMagnetOffset + " to weaken magnets"); yOutput.reset();
      writeBlock(gFormat.format(28), "Z0", "; Lift Z to safe position");
      writeBlock(gFormat.format(90), "; Set absolute distance mode");
      break;

    case "LOAD":
      writeComment(localize("Load dust shoe"));
      writeBlock("; G28 Z0 must be set to most retracted position!!!");
      writeBlock("; Set G30 to dock center location");
      writeBlock(gFormat.format(91), "; Set incremental distance mode");
      writeBlock(gFormat.format(28), "Z0", "; Lift Z to safe position");
...
function onCommand(command) {
  switch (command) {
    ...
    case COMMAND_LOAD_TOOL:
      if (getProperty("useDustBootParking")) {
        //unload dust boot
        dustBoot("UNLOAD");
      }
...
function writeProgramEnd() {
  ...
  onCommand(COMMAND_STOP_SPINDLE);
  // Unload tool after spindle and coolant is off
  if (getProperty("unloadLastTool")) {
    if (getProperty("useDustBootParking")) {
      //unload dust boot
      dustBoot("UNLOAD");
    }
    writeComment(localize("Unload Tool T") + tool.number);
    writeToolBlock("T" + toolFormat.format(getProperty("unloadToolNumber")), conditional(getProperty("useM06"), mFormat.format(6)));
    if (getProperty("useDustBootParking")) {
      //load dust boot
      dustBoot("LOAD");
    }
